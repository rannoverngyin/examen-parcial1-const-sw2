...
export function consultarProductos() {
    group('navegacion-productos', () => {
        ...
        check(listado, {
            'listado HTTP 200': response => response.status === 200,
        });
        ...
        check(total, {
            'total HTTP 200': response => response.status === 200,
        });
    });
...
export function registrarProducto(data) {
    ...
    const ok = check(response, {
        'registro HTTP 200': result => result.status === 200,
    });
...
export function consultarReporte() {
    const response = http.get(`${BASE_URL}/carga/productos/reporte`, {
        tags: {
            endpoint: 'reporte',
        },
    });

    reportLatency.add(response.timings.duration);

    const ok = check(response, {
        'reporte HTTP 200': result => result.status === 200,
    });
...
export function handleSummary(data) {
    return {
        'evidencia/resumen-pico-doble.json':
            JSON.stringify(data, null, 2),

        stdout:
            'Resumen guardado en evidencia/resumen-pico-doble.json\n',
    };
}